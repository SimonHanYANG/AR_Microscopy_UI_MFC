...
CMainFrame::CMainFrame()
{
	...
	m_save_image_prepath = _T("D:\\Simon_workspace\\AR_Microscopy_UI\\tmpImage\\");
	m_save_video_prepath = _T("D:\\Simon_workspace\\AR_Microscopy_UI\\tmpVideo\\");
...
void CMainFrame::PumpMotorControl()
{
	...
	//int m_target_destination_x = 550;
	...
	////CvRect m_ROI_pipette = cvRect(100, 430, 600, 10);
	...
	//	m_pump_motor.MovetoAbsolute(500);
...
// POS: event handler for WM_IMAGE_ACQUIRED 
{
	...
	if (pView->m_is_displaying_image)
	{
		...
		// Gray to Color
		//cv::cvtColor(m_copyImg, m_displayColorImg, cv::COLOR_BGR2GRAY);
...
void CMainFrame::DrawImageResults()
{
	...
	cv::circle(m_displayColorImg, cv::Point(640,400), 1, cv::Scalar(0,0,255), 2);
	cv::Rect m_ROI(240,350,800,100);
	m_cs_display_color_mat.Lock();
	cv::rectangle(m_displayColorImg, m_ROI, cv::Scalar(255, 0, 0));
	m_cs_display_color_mat.Unlock();
...
void CMainFrame::ServoSpermHeadCenter()
{
	...
	//_copyImg = m_copyImg;
	m_cs_copy_mat.Unlock();
	m_current_position = Methods::Vision::FindContours(_copyImg);
...
DWORD CMainFrame::ContactDetectionThread(void* pParameter)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		// move along y-axis
		This->MoveManipLeft(2, i);
		This->WaitForManipLeftStop();
	}
