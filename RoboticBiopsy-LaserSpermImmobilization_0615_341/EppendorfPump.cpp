...
bool EppendorfPump::Initialize()
{
	...
	//Phidget_set_OnAttach_Handler((CPhidgetHandle)stepper, EppendorfPumpCallEventAttachHandler, this);
...
int CCONV EppendorfPumpCallEventAttachHandler(PhidgetHandle MC, void *userptr)
{
	//AfxMessageBox(_T("Motor controller attached!"));
	std::cout << "Motor controller attached!" << std::endl;

	EppendorfPump* This = (EppendorfPump*)userptr;
